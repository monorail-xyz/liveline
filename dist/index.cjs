...
// src/math/matchTime.ts
function findPeriodAtTime(periods, time) {
...
function fmtMinExact(min) {
  const totalSecs = Math.round(min * 60);
  const m = Math.floor(totalSecs / 60);
  const s = totalSecs % 60;
  return `${m}'${s.toString().padStart(2, "0")}"`;
}
function formatMatchMinute(time, periods, scopeId, exact) {
...
var FADE_IN = 0.18;
var FADE_OUT = 0.12;
function drawGrid(ctx, layout, palette, formatValue, state, dt, disableEdgeFade) {
...
function drawMultiCrosshair(ctx, layout, palette, hoverX, hoverTime, entries, formatValue, formatTime, scrubOpacity, tooltipY, tooltipOutline, liveDotX, scoreLine) {
...
var FADE = 0.08;
function drawTimeAxis(ctx, layout, palette, windowSecs, targetWindowSecs, formatTime, state, dt, alignBase) {
...
// src/draw/eventLines.ts
function drawEventLines(ctx, layout, palette, events) {
  ...
  if (opts.hoverX !== null && opts.hoverValue !== null && opts.hoverTime !== null && pts && pts.length > 0) {
    ...
    if (scrubOpacity > 0.01) {
      drawCrosshair(
        ...
        opts.hoverTime,
        opts.formatValue,
        opts.formatTimeCrosshair ?? opts.formatTime,
        scrubOpacity,
        opts.tooltipY,
        lastPt[0],
...
function updateWindowTransition(cfg, wt, displayWindow, displayMin, displayMax, noMotion, now_ms, now, points, smoothValue, buffer) {
  if (wt.to !== cfg.windowSecs) {
    ...
    if (targetVisible.length > 0) {
      const targetRange = cfg.fixedRange ? { min: cfg.fixedRange.min, max: cfg.fixedRange.max } : computeRange(targetVisible, smoothValue, cfg.referenceLine?.value, cfg.exaggerate);
...
function computeMatchEdges(cfg, now, windowSecs, buffer, leftEdge, rightEdge) {
  ...
  const periodEnd = nextPeriod ? nextPeriod.kickoff : period.kickoff + period.duration * 1.1;
  return {
    effectiveLeftEdge: period.kickoff,
    ...
    if (isCandle) {
      ...
      if (transition.startMs > 0 && effectiveMultiSeries.length > 1) {
        ...
        for (const s of effectiveMultiSeries) {
          ...
          if (targetVisible.length > 0) {
            const range = cfg.fixedRange ? { min: cfg.fixedRange.min, max: cfg.fixedRange.max } : computeRange(targetVisible, sv, cfg.referenceLine?.value, cfg.exaggerate);
      ...
      for (const s of effectiveMultiSeries) {
        ...
        if (visible.length >= 2) {
          if (alpha > 0.01) {
            const range = cfg.fixedRange ? { min: cfg.fixedRange.min, max: cfg.fixedRange.max } : computeRange(visible, sv, cfg.referenceLine?.value, cfg.exaggerate);
      ...
      const effectiveFormatTime = cfg.matchTimeline?.periods.length ? (t) => formatMatchMinute(t, cfg.matchTimeline.periods, matchScope) : cfg.formatTime;
      const effectiveFormatTimeExact = cfg.matchTimeline?.periods.length ? (t) => formatMatchMinute(t, cfg.matchTimeline.periods, matchScope, true) : cfg.formatTime;
      let matchAlignBase;
      ...
      const layout = {
        ...
        chartW,
        chartH,
        leftEdge: effectiveLeftEdge,
        rightEdge: effectiveRightEdge,
        minVal,
        maxVal,
        valRange,
        toX: (t) => pad.left + (t - effectiveLeftEdge) / (effectiveRightEdge - effectiveLeftEdge) * chartW,
        toY: (v) => pad.top + (1 - (v - minVal) / valRange) * chartH
      };
      const hoverPx = hoverXRef.current;
      let drawHoverX = null;
      ...
      const computedRange = cfg.fixedRange ? { min: cfg.fixedRange.min, max: cfg.fixedRange.max } : computeRange(visible, smoothValue, cfg.referenceLine?.value, cfg.exaggerate);
      const isWindowTransitioning = transition.startMs > 0;
      const rangeResult = updateRange(
        computedRange,
        rangeInitedRef.current,
      ...
      const effectiveFormatTime = cfg.matchTimeline?.periods.length ? (t) => formatMatchMinute(t, cfg.matchTimeline.periods, singleMatchScope) : cfg.formatTime;
      const effectiveFormatTimeExact = cfg.matchTimeline?.periods.length ? (t) => formatMatchMinute(t, cfg.matchTimeline.periods, singleMatchScope, true) : cfg.formatTime;
      let singleMatchAlignBase;
      if (cfg.matchTimeline?.periods.length) {
        const periods = cfg.matchTimeline.periods;
        if (singleMatchScope) {
...
function Liveline({
  ...
  useLivelineEngine(canvasRef, containerRef, {
    ...
    scoreLabels,
    matchTimeline,
    selectedPeriodId: matchTimeline ? selectedPeriodId ?? void 0 : void 0
  });
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
    ...
    matchTimeline && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("style", { children: `@keyframes liveline-pulse { 0%, 100% { opacity: 1; } 50% { opacity: 0.4; } }` }),
    (effectiveWindows && effectiveWindows.length > 0 || onModeChange || showSeriesToggle) && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: { display: "flex", alignItems: "center", gap: 6, marginBottom: 6, marginLeft: pad.left }, children: [
      effectiveWindows && effectiveWindows.length > 0 && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(
        "div",
        {
          ref: windowBarRef,
          style: {
